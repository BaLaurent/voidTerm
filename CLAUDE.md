...
| Package | Role |
|---|---|
| `com.voidterm.contracts` | Shared interfaces: `VoiceState`, `VoiceInputCallback`, `TranscriptionListener`, `ControlPanel`, `ControlPanelListener`, `DownloadJob`, `FileSpec` |
| `com.voidterm.voice` | Voice system: `VoiceInputManager`, `AudioCapture`, `AudioPreprocessor`, `AudioConfig`, `AudioChunker`, `WhisperBridge`, `WhisperEngine`, `WhisperConfig`, `ParakeetEngine`, `ParakeetConfig`, `ParakeetQuantization`, `TranscriptionOverlay`, `HttpModelDownloader`, `WhisperModelCatalog`, `WhisperModelManager` |
...
**Default engine:** `SettingsDialog.ENGINE_DEFAULT` (= `ENGINE_PARAKEET`) is the single source of truth for the default transcription engine, used as the `getString(KEY_TRANSCRIPTION_ENGINE, …)` fallback in `VoiceInputManager.createEngine()` and `SettingsActivity`. Parakeet is preferred (better accuracy than whisper.cpp on Quest). Neither engine ships a bundled model — `assets/models/` is empty, so a model must be provided regardless of engine (whisper: file picker; Parakeet: in-app download), which is why switching the default introduces no engine-specific regression.

`SettingsActivity` (full-screen Activity, programmatic layout) lets users select a custom whisper.cpp model file via Android's `ACTION_OPEN_DOCUMENT` file picker **or** download one directly via the in-app catalog (see "Model Download" below). `SettingsDialog` holds all preference key constants and label/value arrays. The file-picker path copies the selected file to `{filesDir}/models/`, persists its name in `SharedPreferences` ("voidterm_settings" / "whisper_model_name"), switches the engine to whisper (`KEY_TRANSCRIPTION_ENGINE`), and hot-reloads via `VoiceInputManager.reloadModel()`. Default whisper model name: `ggml-base.bin` (expected in `{filesDir}/models/` — `assets/models/` is empty, no model is bundled). `WhisperBridge.loadModel()` checks `{filesDir}/models/` first, falls back to assets (currently none), and returns a clear error if neither exists. GPU toggle (default off) controls `whisper_context_params.use_gpu`. `WhisperBridge` selects the FP16 library variant at runtime if `Build.VERSION.SDK_INT >= 27` (ARMv8.2-A support). `TermuxActivity.onResume()` calls `applyTheme()` to sync theme after returning from SettingsActivity.

`WhisperCatalogView` (`com.voidterm.app`) renders the whisper model catalog in `SettingsActivity.buildModelSection()` as an accordion grouped by family (tiny / base / small / medium / large). Each row shows the model's size and state: absent → "Download" button; downloaded → "Activate" + delete (🗑); active → "✓ Active". Activating a downloaded model sets `whisper_model_name` and switches the engine to whisper. One download at a time is enforced (guard on `ModelDownloadService.isRunning()`). Progress updates from `BROADCAST_PROGRESS` carry `EXTRA_MODEL_ID` so the view refreshes only the relevant row.

`ParakeetQuantizationView` (`com.voidterm.app`) is a flat 2-row selector in the Parakeet section of `SettingsActivity`, replacing the former single-model status/download/cancel/delete block. Each row corresponds to one `ParakeetQuantization` entry (int8 / fp32) and transitions between three states: absent → "Download" button; downloaded → "Activate" + delete (🗑); active → "✓ Active". The download broadcast (`EXTRA_MODEL_ID` = quantization id "int8"/"fp32") routes progress to the correct row. Rows coexist — downloading or activating one does not remove the other. An fp32 row carries an **adaptive memory note** (`applyFp32Warning`): it reads the device's total RAM (`ActivityManager.MemoryInfo.totalMem`) and recommends per tier (<4 GB not recommended / <8 GB caution / ≥8 GB sufficient), color-coded green (ample) / orange (tight or unsafe) — no hardcoded Quest figure. Auto-activation fires from `ParakeetDownloadJob.onComplete`, so the freshly downloaded quantization becomes active without a manual tap.

### Model Download (Parakeet + Whisper, background)

Both Parakeet and whisper.cpp models download via a **generic foreground service**, `ModelDownloadService` (`com.voidterm.app`, renamed from the former `ParakeetDownloadService`), so downloads survive leaving the Settings screen and app backgrounding. It is separate from `TerminalService` on purpose: `TerminalService.stopIfNoSessions()` would otherwise kill the download.

**Architecture (policy/mechanism split):**
- `ModelDownloadService` is the **mechanism**: foreground lifecycle, progress notification, `PARTIAL_WAKE_LOCK` against device sleep, cancellation, and the generic skip/.tmp/atomic-rename download loop (migrated out of the former `ParakeetModelManager.download()`). It reconstructs the job to run via `DownloadJobs.fromIntent(Context, Intent)` using extras `EXTRA_JOB_TYPE` and `EXTRA_MODEL_ID`.
- `DownloadJob` (interface, `com.voidterm.contracts`) is the **policy**: `id()`, `displayName()`, `files()` → `List<FileSpec>`, `onComplete(Context)`. The plain-data DTO it carries is `FileSpec` (`{url, destFile, label}`, also in `contracts`).
- `HttpModelDownloader` (`com.voidterm.voice`) is the shared low-level HuggingFace HTTP transfer (redirects, 200ms-throttled progress, cancel via `AtomicBoolean`, atomic `.tmp` rename), extracted from the former `ParakeetModelManager` and reused by every job.
- `ParakeetDownloadJob` (`com.voidterm.app`) is parameterized by `ParakeetQuantization`: its factory-type constant is `JOB_TYPE` (= `"parakeet"`); `id()` returns the quantization id (`"int8"` or `"fp32"`). It lists the quantization's files via `ParakeetModelManager.fileSpecs(ctx, q)`; `onComplete` writes `KEY_PARAKEET_QUANTIZATION` (the downloaded quantization id) + `KEY_MODEL_RELOAD_REQUESTED` (full engine reload — NOT a `ParakeetConfig.CONFIG_KEY`).
- `WhisperDownloadJob` (`com.voidterm.app`) lists one ggml file from `WhisperModelCatalog`; `onComplete` activates the model (`whisper_model_name`) and sets `KEY_TRANSCRIPTION_ENGINE=whisper` (Parakeet is the default — without this, activating a whisper model would be invisible to the engine selector).

**Service behavior:**
- `SettingsActivity` starts it via `startForegroundService(ACTION_START_DOWNLOAD)` and observes progress through a private broadcast (`BROADCAST_PROGRESS/COMPLETE/ERROR`, registered `RECEIVER_NOT_EXPORTED` in `onResume`/`onPause`). Broadcasts carry `EXTRA_TEXT`, `EXTRA_PERCENT`, and `EXTRA_MODEL_ID` so the UI can route progress to the correct row. On reopen mid-download it seeds its UI from `ModelDownloadService.isRunning()` + `lastProgressText()`.
- Notification (channel `voidterm_download`, ID **2** ≠ TerminalService's ID 1) shows per-file progress + a **Cancel** action; tapping opens Settings. On complete it posts a dismissable "ready" notification and `stopSelf()`.
- **Cancellation**: `ACTION_CANCEL_DOWNLOAD` trips `cancelFlag`; the download loop checks it between files and inside the byte loop (`InterruptedIOException`), deletes the partial `.tmp`, and teardown is silent (no error notification).
- **Resume**: completed files are skipped; an interrupted file restarts from scratch (no HTTP Range). Parakeet: downloading a quantization is a no-op when all its files are already present — the per-row delete button in `ParakeetQuantizationView` calls `ParakeetModelManager.deleteModels(ctx, q)`, which removes only that quantization's specific files; the shared `nemo128.onnx` and `vocab.txt` are always kept. The delete button is hidden while a download is running for that row and when the quantization has no files. Whisper: per-model delete is in `WhisperCatalogView` rows via `WhisperModelManager.delete()`.
- Manifest: `WAKE_LOCK` permission + a plain `<service>` declaration (no `foregroundServiceType` — `targetSdk 28` doesn't require it).

**Catalog data** (`WhisperModelCatalog`, `com.voidterm.voice`): static registry of 31 ggml variants from `huggingface.co/ggerganov/whisper.cpp` (tiny/base/small/medium/large families, `.en` and quantized variants; quantization is non-uniform — q5_1 for small models, q5_0 for large). Each entry is a `WhisperModel{id, fileName, displayName, sizeMb, family, quantized, englishOnly}`. `WhisperModelManager` tracks filesystem state of downloaded models in `{filesDir}/models/` and provides `isDownloaded`, `listDownloaded`, `delete`, and `nextActiveAfterDelete` (active model falls back to the first remaining downloaded model on deletion).
...
**Quantization selection** (in `loadModel`): `ParakeetEngine` reads `KEY_PARAKEET_QUANTIZATION` (default `"int8"`) via `ParakeetQuantization.byId()` and loads that quantization's encoder and decoder ONNX files from `{filesDir}/models/`; `nemo128.onnx` and `vocab.txt` are shared across quantizations and always loaded from the same path. `KEY_PARAKEET_QUANTIZATION` is written by `ParakeetDownloadJob.onComplete` on successful download, paired with `KEY_MODEL_RELOAD_REQUESTED` to trigger a full engine reload.

`ParakeetQuantization` (static catalog in `com.voidterm.voice`, 2 entries):
- `INT8` (~670 MB) — weights stored inline in the encoder `.onnx`; all 4 ONNX files included in `allFiles()`.
- `FP32` (~2.55 GB) — encoder weights are in a separate `encoder-model.onnx.data` file (2.44 GB) resolved automatically by ONNX Runtime; `allFiles()` includes it as an extra file.

Fields per entry: `id` (`"int8"`/`"fp32"`), `displayName`, `sizeMb`, `encoderFile`, `decoderFile`, `extraFiles`. Helpers: `allFiles()` (shared `nemo128.onnx` + `vocab.txt` + encoder + decoder + extras), `specificFiles()` (encoder + decoder + extras only — used by `deleteModels`), `url(fileName)`, `byId(String)`, `ALL` list, `BASE_URL`.
