...
### Added
- Key/gesture mappings can now control VoidTerm itself, not just the terminal: four new actions — **Next Session** / **Previous Session** (cycle, wrapping both ends), **Sessions Drawer** (toggle the left drawer), and **New Session** — selectable for the Back/Volume single-tap and every double/triple/long/combo gesture slot, alongside the existing Escape / Toggle Keyboard / Macro / Voice Input actions
